...
# 사업 목표
## 최종 목표
- 기업과 연계하여 스마트팜 모니터링 시스템을 개발함으로서 실질적인 기업의 개발 프로세스를 경험하고 이를 이용하여 직접 스마트팜의 제어 및 모니터링 제품을 개발
- OneM2M과 같은 IoT 국제표준을 이용한 산업에서 사용 가능한 시스템 개발을 통해 즉시 직무 수행이 가능한 기술의 습득
## 세부목표
...
# 개발과제
## IoT 표준을 따르는 소규모 스마트팜 시뮬레이터 제작<br/>
1. 실증을 위한 기업 소유의 데이터 수집 장치 기 확보<br/>
2. 실증 전 사전에 개발과정에 필요한 소규모 시뮬레이터 제작<br/>
3. 소규모 시뮬레이터 제작 -시스템 구축 과정에서 활용<br/>
...
## 센서 데이터 처리용 미들웨어 제작
1. IoT 표준기술 (oneM2M) - 온실의 센싱 데이터 처리 및 전송을 위한 oneM2M Gateway Platform- 온실의 데이터 수집 및 관리를 위한 oneM2M Server Platform
...
2. 미들웨어 구조
       - 센서에서 수집된 데이터를 안정적으로 관리하고 오류데이터 및 센서의 오작동 검출
       - 다량의 센싱 데이터를 안정적으로 처리할 수 있도록 최적화 알고리즘 선택
       - 데이터베이스를 연동하여 센서에서 수집한 데이터를 저장하고 통합시스템과 연동
       - MongoDB(NoSQL) 사용
...
3. IoT 표준 oneM2M 기반 Gateway Platform 구축
       - 시스템 설정 관리, 장애 관리, 로그 관리, Open API 사용 이력 관리기능 제작
       - HTTP, CoAP, MQTT 프로토콜을 지원하며 서비스 계층 Core Protocol과의 Binding 기능
       - 디바이스/어플리케이션/사용자 등록 요청 시 공통 플랫폼에서 인증키를 생성/발급/관리 기능
       - HTTP, CoAP, MQTT 간 프로토콜 변환(Conversion) 기능 구축
       - RFC7252의 CoAP-HTTP Proxy를 만족하는 CoAP과 HTTP간의 프로토콜 변환기
       - 경량 프로토콜(CoAP, MQTT 등)과 비경량 프로토콜(HTTP, SOAP 등) 간의 통신을 위한 Proxy 구조 설계 구성요소 식별
       - CoAP/MQTT 간 프로토콜 변환 기능
       - MQTT-HTTP 간 프로토콜 변환을 위한 요구사항 도출/구조 설계 및 프로토콜 변환 기능
...
4. IoT 표준 oneM2M 기반 Server Platform 구축
       - IoT 국제 표준인 oneM2M을 분석하여 디바이스로부터 전송되는 데이터를  수집할 수 있도록 제작
       - 관리되는 모든 리소스는 RESTful API로 접근할 수 있도록 제작
       - URI 형태를 준수
       - Resource 맵핑 URI식별자 구성
...
1. 환경정보를 모니터링하고 적정 수치범위 초과시 원격제어 할 수 있는 시스템 개발
2. 제어 대상 장비를 통합제어기를 통해 웹에서 시스템으로 제어하도록 구성
       - 제어장비 적정수치 설정 및 변경관리
       - 제어장비 운영정보 및 센서데이터 통합 조회
...
3. 온실 통합제어기 간 RS485기반 Modbus 인터페이스 제작
       - 유리온실의 센서노드 및 구동기 노드, 온실 통합제어기 간 표준화된 방식의 프로토콜을 사용하여 서로 다른 장치 간 상호 연동이 가능하도록 제작
       - 반밀폐형 유리온실을 구성하는 각 구성 요소나 장치는 ID를 통해 식별되기 때문에 RS485 Modbus 방식으로 데이터 송수신을 위해서 address 사용
...
<br/><br/>
       ...
       - 상호연동을 위해 기존에 사용하던 ID와 RS485 Modbus address간 매핑
...
4. 유리온실 통합관제시스템 구축
       ...
       - 유리온실 통계정보 조회
       - 유리온실 데이터 관리기능 제작
       - 각 센서 값에 대한 일별 평균치, 최고치, 최저치, 합을 계산
...
## 기업 스마트팜을 이용한 시스템 실증
1. 연계 기업의 실증지를 방문하여 대규모의 스마트팜 환경에서 미리 구축한 개발환경을 이용하여 시스템이 실제로 데이터를 받아서 잘 작동하는지 확인
<br/><br/>
